...
const port = Number(process.env.LLLOGS_PORT ?? 7331);

Bun.serve({ port, fetch: app.fetch });
// Boot stats are best-effort: a readonly DB that predates the schema (e.g. `serve` before any
...
process.stdout.write(`lllogs v2 → http://localhost:${port}\n`);
