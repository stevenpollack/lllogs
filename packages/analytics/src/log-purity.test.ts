 ...
 * CLI; it never imports bun:sqlite or @clogdy/ingest. The fixture DB is built by
 ...
 * only (when CLOGDY_LOG_DIR is set) and otherwise stay SILENT — never a byte to
...
beforeAll(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "clogdy-purity-"));
  ...
  const env = { ...process.env, CLOGDY_LOG_DIR: logDir, CLOGDY_LOG_LEVEL: "debug" };
  const run = Bun.spawnSync(
    ["bun", "run", "v2:analytics", "--", "--db", tmpDb, "--metric", "toolCounts"],
    { cwd: REPO_ROOT, env },
  );
...
describe("analytics logging purity (Phase 4)", () => {
  ...
  it("keeps stdout pure JSON with CLOGDY_LOG_DIR + debug logging", () => {
    ...
    expect(metricStderr).not.toContain("v2:analytics:");
  ...
  // Test 3 — without CLOGDY_LOG_DIR the analytics logger is SILENT (even at debug):
  ...
  it("stays silent without CLOGDY_LOG_DIR — no log bytes on stdout or stderr", () => {
    // Drop CLOGDY_LOG_DIR (it may be set in the runner's own env) so the child
    // truly runs without a log directory.
    const { CLOGDY_LOG_DIR: _drop, ...baseEnv } = process.env;
    const env = { ...baseEnv, CLOGDY_LOG_LEVEL: "debug" };
    const run = Bun.spawnSync(
      ["bun", "run", "v2:analytics", "--", "--db", tmpDb, "--metric", "toolCounts"],
      { cwd: REPO_ROOT, env },
    );
