 ...
 *  - LLLOGS_LOG_DIR set → `<dir>/<proc>[-<pid>].jsonl` (sync append; analytics gets per-pid because
 ...
 * Level from LLLOGS_LOG_LEVEL (default "info", validated — an unknown value must NOT throw at construction).
 */
export function nodeLogger(proc: string): Logger {
  const envLevel = process.env.LLLOGS_LOG_LEVEL as LogLevel | undefined;
  const level: LogLevel = envLevel && LOG_LEVELS.includes(envLevel) ? envLevel : "info";
  const dir = process.env.LLLOGS_LOG_DIR;
