...
// spawn, which inherit this env — read LLLOGS_LOG_LEVEL when the logger is built.
...
// `LLLOGS_LOG_LEVEL=debug bun test`) can still opt into logs. Tests that assert ON
// logs (e.g. analytics log-purity) pass their own LLLOGS_LOG_LEVEL to the children
// they spawn, so this default never hides what they're checking.
if (!process.env.LLLOGS_LOG_LEVEL) process.env.LLLOGS_LOG_LEVEL = "silent";
